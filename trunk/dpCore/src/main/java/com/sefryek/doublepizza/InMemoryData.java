...
import com.sefryek.doublepizza.core.helpers.CombinedMenuItemPricing;
...
public class InMemoryData {
    ...
    //Saeid AmanZadeh
    public static Boolean runTimeEnReload = false;
    public static Boolean runTimeFaReload = false;
    //--
    private static Logger logger = Logger.getLogger(InMemoryData.class);
    private static List<Menu> menuList;
    private static List<Store> storeList;
    private static List<Store> deviceStoreList;
    ...
    private static List<Slide> sliderList;

    public static List<Store> getDeviceStoreList() {
        return deviceStoreList;
    }

    public static void setDeviceStoreList(List<Store> deviceStoreList) {
        InMemoryData.deviceStoreList = deviceStoreList;
    }

    public static List<Slide> getSliderList() {
    ...
    public void writeTBLFileForTEST() {
        String dirPath = ArchiveHandler.createDirectory();
        if (dirPath == null) {
            logger.error("error on create archive directory because dirPath is null");
            return;
...
//        Serializer.writeObject(Constant.TABLE_LIST.StoreHours.toString(), storeHoursTBL, dirPath);
    ...
    public void loadData() throws DataLoadException {
        ...
        //By Saeid AmanZadeh - reload menu data correctly
//        System.out.println("---------En---------"+englishDataHashMap.size());
//        System.out.println("-------Fa-----------"+frenchDataHashMap.size());
        //Saeid AmanZadeh
...
//                              writeTBLFileForTEST();
            ...
            List<Store> storeListTemp = createStoresList(false, null);
            //List<Store> deviStoreListTemp = createStoresList(true);

            orderMenu.setCategoryList(menuService.getMenuCategoryList(InMemoryData.getWebCategoriesTBL()));
            ...
            //deviceStoreList = deviStoreListTemp;
            ...
            throw new DataLoadException(e.getCause(), e.getClass(), "Data has not been load successfully!");
    ...
    public static List<ToppingCategory> getDistinctToppingsCategoryList(
        ...
        try {
            ...
            throw new DAOException(e, e.getClass(), "can not execute getToppingsCategoryList service for single: " + singleItemID
                    + " in combined: " + combinedMenuItemID + "\nCause: " + e.getMessage());
        ...
        if (toppingCategories !=null){
            ...
            for (int i = 0; i < topping_categoryTBL.size(); i++) {
                ...
                if (row[row.length - 1] == null) {
                    logger.error("The Web seq is null. row" + row.toString());
                }
                minSequenceIndex = i;
...
//                    System.out.print(currentRow[ii] + "###");
//                }
    ...
    public static void fillCombinedItem(CombinedMenuItem combinedMenuItem
            , boolean isTwoForOne) throws Exception {
        logger.debug("Start of fillCombinedItem combinedMenuItem ProductNo:"+combinedMenuItem.getProductNo());


        if (Integer.parseInt(combinedMenuItem.getProductNo()) == 148)
            logger.debug("combinedMenuItem.getProductNo() is 148");
        ...
        try {
            ...
            if (Integer.parseInt(combinedMenuItem.getProductNo()) == 13){
                allProdLinkRowList = getJoinedGroupId(combinedMenuItem.getProductNo());
            }
            for (int k = 0; k < allProdLinkRowList.size(); k++) {
                ...
                /**
                 * for each group of modifiers in prodlink there are one or more default items. if there are more than
                 * one default items for example there are 3 default items, it means this group has to be repeated 3 times
                 * inside the combined and each one has a defualt item among these 3 defaults.
                 */
                List<String> defaultsingleItemIds = getDefaultItemIdsForGroup(combinedMenuItem.getProductNo(),
                        prodLinkRow.get(Constant.TABLE_LIST.ProdLink + "." + Constant.PRODLINK_MODGROUPNO).toString());
                ...
                if (forced.equals(Constant.VALUE_IS_FORCED)) {
                    ...
                    if (modifierRowList != null && modifierRowList.size() != 0) {

                        List<MenuSingleItem> allsingles = new ArrayList<MenuSingleItem>();
                        List<String> itemTypes = new ArrayList<String>();
                        ...
                        for (Map<String, Object> aModifierRow : modifierRowList) {
                            if (Boolean.valueOf(aModifierRow.get(
                                ...
                                logger.debug("in fillCombinedItem toppingRows:"+toppingRows);
                                logger.debug("in fillCombinedItem  menuSingleItem Id:"+ menuSingleItem.getId());
                                if (toppingRows!=null && menuSingleItem!=null)         {
                                ...
                                if (isTwoForOne) {
                                    ...
                                    allsingles.add(menuSingleItem);
                                    itemTypes.add((String) aModifierRow.get(
                        ...
                        if (isTwoForOne && allAlternatives.size() != menuSingleItems.size()) {
                            /**
                             * Eqaul size for allAlternatives and menuSingleItems means that no menuSinleItem has been
                             * added to the list in last step. so there is no need for adding an alternatives list for it
                             */
                            ...
                            MenuSingleItem defaultSingleItem = null;
                            String defaultItemType = null;
                            itemAlternatives = new ArrayList<MenuSingleItem>();

                            for (String defaultSingleItemId : defaultsingleItemIds) {
                                int foundedItemIndex = -1;
                                for (int f = 0; f < allsingles.size(); f++) {
                                    MenuSingleItem menuSingleItem = allsingles.get(f);
                                    if (defaultSingleItemId.equals(menuSingleItem.getId())) {
                                        foundedItemIndex = f;
                                        defaultSingleItem = menuSingleItem;
                                        defaultItemType = itemTypes.get(f);
                                        break;
                                    }
                                ...
                                for (MenuSingleItem aMenuSingleItem : allsingles)
                                    itemAlternatives.add(aMenuSingleItem);

                                itemAlternatives.remove(foundedItemIndex);
                                menuSingleItems.add(defaultSingleItem);
                                itemCaptions.add(getToppingOrCaptionTitleKey(defaultItemType, combinedMenuItem.getProductNo()));
                                allAlternatives.add(itemAlternatives);


                            }

/*                            String typeOfDefaultItem = null;
                            for (int menuSingleIndex = 0; menuSingleIndex < allsingles.size(); menuSingleIndex++) {
                                MenuSingleItem menuSingleItem = allsingles.get(menuSingleIndex);
                                foundDefaultSingleItem = false;
                                int defaultSingleItemIdIndex = 0;
                                for (; defaultSingleItemIdIndex < defaultsingleItemIds.size(); defaultSingleItemIdIndex++) {
                                    String defaultSingleItemId = defaultsingleItemIds.get(defaultSingleItemIdIndex);
                                    if (defaultSingleItemId.equals(menuSingleItem.getId())) {
                                        foundDefaultSingleItem = true;
                                        typeOfDefaultItem = itemTypes.get(menuSingleIndex);
                                        iterationOfDefaultSingleItem++;
                                    }
                                }
...
//                                }
                                ...
                                if (foundDefaultSingleItem) {
                                    String caption;
                                    //TODO: we don't count pizzas any more, so itemTypes of pizza will not cotains word like first, second etc.
//                                        if (typeOfDefaultItem.equals(Constant.ITEM_TYPE_PIZZA) && iterationOfMenuSingleItem > 1) {
//                                            numberOfPizzas++;
//                                            caption = getPizzaCaptionKey(typeOfDefaultItem, combinedMenuItem.getProductNo(), numberOfPizzas);
//
//                                        } else
                                    caption = getToppingOrCaptionTitleKey(typeOfDefaultItem, combinedMenuItem.getProductNo());
                                    defaultSingleItem = menuSingleItem;
                                    typeOfDefaultItem = itemTypes.get(menuSingleIndex);
                                    defaultsingleItemIds.remove(defaultSingleItemIdIndex);
                                    itemAlternatives = new ArrayList<MenuSingleItem>();

                                    for (MenuSingleItem aMenuSingleItem : allsingles)
                                        itemAlternatives.add(aMenuSingleItem);

                                    itemAlternatives.remove(menuSingleIndex);
...
//            if (menuSingleItems == null || menuSingleItems.size() == 0)
//                throw new Exception("there is no main single item for current combined");
//
//            if (menuSingleItems.size() != allAlternatives.size())
//                throw new Exception("Number of main single items and alternatives is not equal." + "\n"
//                        + "main single items: " + menuSingleItems.size() + "    alternatives no: " + allAlternatives.size());
//
//            if (menuSingleItems.size() != itemCaptions.size())
//                throw new Exception("Number of main single items and captions is not equal." + "\n"
//                        + "main single items: " + menuSingleItems.size() + "    captions no: " + itemCaptions.size());
            ...
            logger.debug("End of  fillCombinedItem combinedMenuItem ProductNo:"+combinedMenuItem.getProductNo());


        } catch (Exception e) {
            e.printStackTrace();
            throw new Exception("Can not execute fillCombinedMethod for combined with id = " + combinedMenuItem.getProductNo()
                    + "\nCause: " + e.getMessage());
        }
                ...
                if (row.get(Constant.MENUITEM_PRODUCTNO) != null) {
                    if (!row.get(Constant.MENUITEM_PRODUCTNO).toString().equals("")) {
                        if (Integer.parseInt(row.get(Constant.MENUITEM_PRODUCTNO).toString()) == Integer.parseInt(productId)) {
                            ...
                            logger.debug("prodLinkMap for :"+productId+" ="+prodLinkMap);
                    ...
                    if (modlinkRow.get(Constant.PRODLINK_MODGROUPNO) != null) {
                        if (!modlinkRow.get(Constant.PRODLINK_MODGROUPNO).toString().equals("")) {
                            if (Integer.parseInt(modlinkRow.get(Constant.PRODLINK_MODGROUPNO).toString()) ==
                                ...
                                //change keySets of rowMap to set tablaName as prefix
                                Map<String, Object> modLinkMap = new HashMap<String, Object>();
    ...
    public static MenuSingleItem getMenuSingleItemInstanceForSuggestion(Map tableRow, String parentID, BigDecimal price) {
        ...
        if (modifierId != null && !modifierId.equals("")) {
            ...
            Boolean isRedeemable = (Boolean) tableRow.get(Constant.MODIFIER_ISREDEEMABLE);
            menuSingleItem = new MenuSingleItem(modifierId, MenuSingleItem.IdType.ModifierID,
                    ...
                    Integer.parseInt(tableRow.get(Constant.MODIFIER_WEBSEQ).toString()),isRedeemable);
    ...
    public static MenuSingleItem getMenuSingleItemInstance(Map tableRow, String parentID, String tableType) {
        ...
        if (tableType.equals(("" + Constant.TABLE_LIST.Modifier))) {
            ...
            if (modifierId != null && !modifierId.equals("")) {
                ...
                Boolean isRedeemable = (Boolean) tableRow.get(Constant.TABLE_LIST.Modifier + "."+Constant.MODIFIER_ISREDEEMABLE);
                menuSingleItem = new MenuSingleItem(modifierId, MenuSingleItem.IdType.ModifierID,
                        ...
                        Integer.parseInt(tableRow.get(Constant.TABLE_LIST.Modifier + "." + Constant.MODIFIER_WEBSEQ).toString()),isRedeemable);
                if (menuSingleItem != null){
                    return menuSingleItem;
                }   else{
                    logger.debug("in getMenuSingleItemInstance modifierId:"+modifierId);
                    return null;
                }
            ...
            Boolean isRedeemable = (Boolean) tableRow.get(Constant.MENUITEM_ISREDEEMABLE);
            menuSingleItem = new MenuSingleItem(productNo,
                    ...
                    Integer.parseInt(tableRow.get(Constant.MENUITEM_SEQUENCE).toString()),isRedeemable);
    ...
    public static Topping getToppingInstance(Map tableRow, String parentID) {
        ...
        /**
         * price of a topping comes from modlink.price. but if this price is zero, we have to get price from corresponding prodlink.price
         * which it is possible to be zero as well. but this is the end.
         */
        BigDecimal toppingPrice = new BigDecimal(tableRow.get(Constant.TABLE_LIST.ModLink + "." + Constant.MODLINK_PRICE).toString());
        if (toppingPrice.compareTo(new BigDecimal(0)) == 0)
            toppingPrice = new BigDecimal(tableRow.get(Constant.TABLE_LIST.ProdLink + "." + Constant.PRODLINK_PRICE).toString());


        return new Topping(
                toppingID, toppingPrice, nameKey,
...
//a subcategory object can contain 3 type of objets[CombinedMenuItem, MenuSingleItem, Category],
    ...
    public static MenuSingleItem findMenuSingleItemByIdAndGroupId(String id, String groupId) {
        ...
        if (groupId == null) {
            ...
            for (MenuSingleItem menuSingleItem : allMenuSingleItem) {
                if ((menuSingleItem.getId().equals(id)) && (menuSingleItem.getGroupId() != null || menuSingleItem.getGroupId().equals("") || menuSingleItem.getGroupId().isEmpty()))
                    return menuSingleItem;
            }
    ...
    public static Menu getItemByName (String name) {
        Menu menuItem = null;
        for (Menu menu : menuList) {
            if (menu.getName().equals(name)){
                menuItem = (Menu) menu.clone();
                return menuItem;
            }
        }
        return null;
    ...
    public static List<Category> getFoodByTypeInMenuList(String name) {
        List<String> drinkCatIdList = Arrays.asList("19");
        List<String> sideOrdeCatIdList = Arrays.asList("15","16","18");
        Menu menuItems = new Menu(null);
        List<Category> menuCategories = new ArrayList<Category>();
        menuItems = getMenuByName("menu");
        if (name.equals("menu")) {
            for (Category category : menuItems.getCategoryList()) {
                if (!drinkCatIdList.contains(category.getId()) && !sideOrdeCatIdList.contains(category.getId())) {
                    menuCategories.add(category);
                }
            }
        } else if (name.equals("Side Orders")){
            for (Category category : menuItems.getCategoryList()) {
                if (sideOrdeCatIdList.contains(category.getId())) {
                    menuCategories.add(category);
                }
            }
        } else if (name.equals("Drinks")){
            for (Category category : menuItems.getCategoryList()) {
                if (drinkCatIdList.contains(category.getId())) {
                    menuCategories.add(category);
                }
            }
        }
...
//get preSelected SingleItemID per each CombiendMenuItem
    ...
    public static List<String> getDefaultItemIdsForGroup(String CombinedMenuItemID, String modGroupNo) {
        ...
        try {
            for (Map<String, Object> presetRow : modifier_preset_JoinTBL) {
                if (presetRow.get(Constant.PRESET_BASEITEM) != null) {
                    if (Integer.parseInt(presetRow.get(Constant.PRESET_BASEITEM).toString()) == 148) {
                        logger.debug("presetRow.get(Constant.PRESET_BASEITEM).toString() is 148");
                    }
                    if (Integer.parseInt(presetRow.get(Constant.PRESET_BASEITEM).toString()) == Integer.parseInt(CombinedMenuItemID)
                            && Integer.parseInt(presetRow.get(Constant.PRESET_MODGROUPNO).toString()) == Integer.parseInt(modGroupNo)) {
                        specialSingleItemIds.add(presetRow.get(Constant.MODIFIER_MODIFIERID).toString());
                    }
                }
            }
        } catch (Exception e) {
            logger.error("can not execute getDefaultItemIdsForGroup method. cause: " + e.getMessage() + " on productNo: " + CombinedMenuItemID);
        }
    ...
    public static List<Store> createStoresList(Boolean needContext, ServletContext servletContext) {
        ...
        for (Map<String, Object> storeRow : storeMastTBL) {
            ...
            try {
                ...
                if(needContext){
                    ...
                    String prefixURL = context + InMemoryData.getImageServicePath(Constant.DATA_RESOURCES_STORES_PATH);;
                    imageUrl = (String) prefixURL + storeRow.get(Constant.STOREMAST_IMAGEURL);
                    if (imageUrl.equals(prefixURL) || storeRow.get(Constant.STOREMAST_IMAGEURL).equals("null"))
                        imageUrl = prefixURL + "branch.png";
                }
                else {
                    imageUrl = (String) storeRow.get(Constant.STOREMAST_IMAGEURL);
                    if (imageUrl == null || imageUrl.isEmpty() || storeRow.get(Constant.STOREMAST_IMAGEURL).equals("null"))
                        imageUrl = "branch.png";
                }
        ...
        try {
            ...
            for (Suggestion suggestion : suggestionList) {
                ...
                if (menuSingleItem != null) {
                    ...
                    clonedMenuSingleItem.setIsRedeemable(true);
                    suggestion.setMenuSingleItem(clonedMenuSingleItem);
                } else
                    logger.warn("on createSuggestionList: not found menuSingleItem for suggestion modifierId: " +
                            modifierId + " productNo: " + suggestion.getProductNo());
            }
    ...
    public static List<Store> getAllDeviceStores(ServletContext context) {
        return createStoresList(true, context);
    }
    ...
    public static BigDecimal getCombinedRealPrice(CombinedMenuItem combined) {
        return new CombinedMenuItemPricing(combined).getDisplayingPrice();
    }
    ...
    public static boolean CombinedIsSpecial(CombinedMenuItem combinedMenuItem) {
        Menu special = getMenuByName(Constant.SPECIAL_MENU_NAME);
        List<Category> categoryList = special.getCategoryList();
        for (Category category : categoryList) {
            List<SubCategory> subCategoryList = category.getSubCategoryList();
            for (SubCategory subCategory : subCategoryList) {
                if (subCategory.getType() == CombinedMenuItem.class) {
                    if (combinedMenuItem == subCategory.getObject())
                        return true;
                }
            }
        }
...
//        return true;
    ...
    public static void addTOLangualHashmaps(String key, String englishWord, String frenchWord) {
        ...
        //Saeid AmanZadeh
        if (englishDataHashMap == null || runTimeEnReload){
            englishDataHashMap = new HashMap<String, String>();
            runTimeEnReload = false;
        }
        if (frenchDataHashMap == null  || runTimeFaReload){
            frenchDataHashMap = new HashMap<String, String>();
            runTimeFaReload = false;
        }
        //--
...
//        return (toppingName != null) && (!toppingName.equalsIgnoreCase("free") & !toppingName.equalsIgnoreCase("dairy"));
        return (toppingName != null) && (!toppingName.equalsIgnoreCase("free"));
    }
    ...
    public static Integer getSingleItemNumberOfToppings(String id, String groupID) {

        if (groupID == null || groupID.equals("")) {
            for (Map<String, Object> modifier : modifierTBL) {
                if (Integer.parseInt(id) == Integer.parseInt((String) modifier.get(Constant.MODIFIER_MODIFIERID))) {
                    return (Integer) modifier.get(Constant.MODIFIER_NUMTOPPING);

                }
            }
        }
        logger.error("error on finding NumberOfToppings for menuSingleItem with id:" + id);
        return -1;
    }
...
//                    if ((part != null && part.equals(Constant.TOPPING_PART_FULL)) || part == null)
    ...
    public static void removeDefaultExclusiveToppings(MenuSingleItem menuSingleItem, Map<Integer, String> selectedToppings) {
        if (selectedToppings == null)
            return;
        ...
        for (Topping topping : defaultToppings) {
            if (!isExclusiveTopping(menuSingleItem, topping))
                continue;
...
//                    if ((part != null && part.equals(Constant.TOPPING_PART_FULL)) || part == null)
    ...
    public static String getParentCategoryId(Integer sigleChildId) {
        ...
        for (Menu aMenuList : menuList) {
            List<Category> categories = (aMenuList).getCategoryList();
    ...
    public static Boolean isExclusiveTopping(MenuSingleItem menuSingleItem, Topping topping) {
        List<ToppingCategory> toppingCategoryList = menuSingleItem.getToppingCategoryList();
        if (toppingCategoryList == null)
            return null;

        for (ToppingCategory category : toppingCategoryList) {
            if (!category.getExclusive())
                continue;

            if (findToppingByIdInCategory(topping.getId(), category) != null)
                return true;
        }
        return false;
    }
