...
public class OrderDAO extends BaseDAO {
    ...
    private static final String Insert_To_Web_Header =
            "INSERT INTO Web_Header (Docnumber, Takeout, Cust, Ext, Todaydate, Store, Total, DiscountDesc, Discount) " +
                    " VALUES (:docnumber, :takeout, :cust, :ext, :todaydate, :store, :total, :discountdesc,  :discount)";

    private static final String Insert_To_Web_Cust =
            "INSERT INTO webCust (DocNumber, Lastname, Phone," +
                    " Street, Apt, City, Pc, ext, " +
                    "Ccode, Building, Room,Takeout, Paytype, isRegistered, Notes, Store, Num, order_from) " +
    ...
    public void save(Order order,ContactInfo contactInfo, String pickupPhone, String pickupExt) throws DAOException {
        String note="";
        if(order.getOrderNote()!=null && order.getOrderNote()!=""){
        ...
        try {
            ...
            String pickExt="";
            if (pickupPhone!=null){
                pickPhone=pickupPhone;
                pickExt=pickupExt;

            } else {
              pickPhone=contactInfo.getPhone();
              pickExt=contactInfo.getExt();
            }
            if (order.getDiscount() == null) {
                order.setDiscount(Double.valueOf(0));
            }
            ...
            session.createSQLQuery(Insert_To_Web_Header)
                    ...
                    .setString("cust", pickPhone)
                    .setString("ext", pickExt)
                    ...
                    .setString("discountdesc", order.getDiscountDesc())
                    .setDouble("discount", order.getDiscount())
                    .executeUpdate();

            //create web cust item
            session.createSQLQuery(Insert_To_Web_Cust)
                    .setString("docnumber", docnumber)
                    .setString("lastname",user.getOfficaialTitle().name()+' '+user.getFirstName()+' '+user.getLastName())
                    .setString("phone", contactInfo.getPhone())
                    .setString("street", contactInfo.getStreet())
                    .setString("apt", contactInfo.getSuiteAPT())
                    .setString("city", contactInfo.getCity())
                    .setString("pc", contactInfo.getPostalcode())
                    .setString("ext", contactInfo.getExt())
                    .setString("ccode", contactInfo.getDoorCode())
                    .setString("building", contactInfo.getBuilding())
                    .setString("room", "")
                    .setBoolean("takeout", order.getDeliveryType() == Order.DeliveryType.PICKUP)
                    .setString("paytype", order.getPaymentType())
                    .setBoolean("isregistered", user.getIsRegistered())
                    .setString("notes",note)
                    .setString("store", store)
                    .setString("order_from",order.getOrderFrom())
                    .setString("num",contactInfo.getStreetNo())
                    .executeUpdate();
