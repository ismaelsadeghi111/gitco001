...
//            basketSingleInCombined.setGroupId(singleAltGroupId);
    ...
    public ActionForward getPreSelectedToppings(ActionMapping mapping, ActionForm form, HttpServletRequest request,
        ...
        String itemName = "";
        String imageAddress = "";
        List<BasketSingleItem> basketSingleItemList = null;

        HttpSession session = request.getSession();
        BasketCombinedItem basketCombinedItem = (BasketCombinedItem) session.getAttribute(Constant.BASKET_COMBINED_IN_SESSION);
        if(basketCombinedItem != null && basketCombinedItem.getBasketSingleItemList() != null){
         basketSingleItemList = basketCombinedItem.getBasketSingleItemList();
        }
        BasketSingleItem basketSingleItem = null;

        if (basketSingleItemList != null && basketSingleItemList.size()>0) {
        ...
        if (basketSingleItem != null) {
            ...
            if (basketCombinedItem !=null && basketCombinedItem.getGroupIdRef() != null){
             combinedMenuItem = InMemoryData.findCombinedMenuItemByProdNoAndGroupId(basketCombinedItem.getProductNoRef(), basketCombinedItem.getGroupIdRef());
            }
            if(combinedMenuItem != null){
            imageAddress = combinedMenuItem.getImageURl();
            price = null;
